...
def getNewHiddenService():
    ...
    cursor.execute("INSERT INTO Links (Url, CreatedOn, HiddenServiceId, IsIndex) VALUES (%s, %s, %s, %s)",
                   (result[0][1], datetime.datetime.now(), result[0][0], 1))
...
def saveLink(link, title, content, status, isNew = 1):
    ...
    if status == 4: #In case it's not a text/html
        if content[0] is not None:
            ...
            cursor.execute("UPDATE Links SET Status=4, WHERE Id=%s", (link[0], ))
        db.commit()
        db.close()
        return
